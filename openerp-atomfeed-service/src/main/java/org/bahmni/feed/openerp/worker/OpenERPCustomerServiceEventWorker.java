...
public class OpenERPCustomerServiceEventWorker implements EventWorker {
    ...
    private List<Parameter> mapParameters(OpenMRSPatient openMRSPatient, String eventId, String feedUri, boolean isFailedEvent) {
        ...
        parameters.add(createParameter("gender", openMRSPatient.getGender(), "string"));
        DateFormat df = new SimpleDateFormat("MM/dd/yyyy");
        logger.info(String.format("birthdate [%s]",openMRSPatient.getBirthdate()));
        logger.info(String.format("registrationdate [%s]",openMRSPatient.getDateCreated()));
        String birthdateString = "";
        String registrationdateString = "";
        if (openMRSPatient.getBirthdate() != null) {
            birthdateString = df.format(openMRSPatient.getBirthdate());
        }
        if (openMRSPatient.getDateCreated() != null) {
            registrationdateString = df.format(openMRSPatient.getDateCreated());
        }
        
        parameters.add(createParameter("birthdate", birthdateString, "string"));
        parameters.add(createParameter("registrationdate", registrationdateString, "string"));       
